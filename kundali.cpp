...
<div class="card">
  ...
  <div class="section">
    ...
    <div><span class="zodiac-badge">)";
    html << k["zodiac_sign"].get<string>();
    html << R"(</span></div>
  </div>
 
  <div class="section">
    <label>Lucky Quotes</label>
    <ul class="quotes">)";
    for (auto& q : k["lucky_quotes"]) {
        html << "<li>" << q.get<string>() << "</li>";
    }
    html << R"(</ul>
  </div>
...
int main() {
    ...
    // ── POST /kundali → JSON (original behaviour) ─────────────────────────────
    ...
    svr.Post("/kundali/txt", [](const httplib::Request& req, httplib::Response& res) {
        ...
        string txt = generateTXT(k);
        string name = k["name"].get<string>();
        string safe = name;
        for (char& c : safe) if (!isalnum(c)) c = '_';
 
        res.set_header("Content-Disposition",
            "attachment; filename=\"" + safe + "_kundali.txt\"");
        res.set_content(txt, "text/plain");
    });
 
    // ── POST /kundali/pdf → HTML page with print-to-PDF button ───────────────
    svr.Post("/kundali/pdf", [](const httplib::Request& req, httplib::Response& res) {
        ...
        json k = buildKundali(body);
        string html = generatePDFHTML(k);
        res.set_content(html, "text/html");
    });
 
    // ── POST /kundali/save → save .txt on server + return JSON ───────────────
